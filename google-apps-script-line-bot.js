...
function processUserMessage(userId, msg) {
    ...
    if (cleanMsg.includes("ดวงวันนี้") || cleanMsg === "ดวง" || cleanMsg.includes("พยากรณ์")) {
        // คำนวณตามราศีของสมาชิก หรือถ้าไม่ได้ระบุให้ใช้ราศีเมษ/หมุนเวียน
        const userZodiac = memberInfo && memberInfo.zodiac ? parseInt(memberInfo.zodiac, 10) : 1;
        const fortuneObj = getZodiacFortune(userZodiac, new Date());
        return formatFortuneMessage(userZodiac, fortuneObj);
    }
...
/**
 * ✨ ดึงพยากรณ์ดวงตามหลักมหาทักษาปกรณ์ (คำนวณตามดาวประจำวันจริง เปลี่ยนแปลงทุกวัน 100%)
 */
function getZodiacFortune(zodiacNum, targetDate) {
    const dateObj = targetDate || new Date();
    const dayOfWeek = dateObj.getDay(); // 0=อาทิตย์, 1=จันทร์, ..., 6=เสาร์

    const ZODIAC_RULERS = {
        1: { name: "ราศีเมษ", ruler: 3, icon: "♈" },
        2: { name: "ราศีพฤษภ", ruler: 6, icon: "♉" },
        3: { name: "ราศีเมถุน", ruler: 4, icon: "♊" },
        4: { name: "ราศีกรกฎ", ruler: 2, icon: "♋" },
        5: { name: "ราศีสิงห์", ruler: 1, icon: "♌" },
        6: { name: "ราศีกันย์", ruler: 4, icon: "♍" },
        7: { name: "ราศีตุลย์", ruler: 6, icon: "♎" },
        8: { name: "ราศีพิจิก", ruler: 3, icon: "♏" },
        9: { name: "ราศีธนู", ruler: 5, icon: "♐" },
        10: { name: "ราศีมังกร", ruler: 7, icon: "♑" },
        11: { name: "ราศีกุมภ์", ruler: 8, icon: "♒" },
        12: { name: "ราศีมีน", ruler: 5, icon: "♓" }
    };
    ...
    const TAKSA_MEANINGS = {
        0: {
            name: "บริวาร",
            variations: [
                { desc: "มีเกณฑ์ต้องเข้าไปเกี่ยวข้องกับคนหมู่มาก คนรอบข้างหรือบริวารจะให้ความร่วมมือดี การทำงานเป็นทีมจะส่งผลลัพธ์ที่ยอดเยี่ยม", good: "การทำงานเป็นทีม, ได้มิตรภาพ", bad: "การตามใจคนอื่นมากเกินไป" },
    ...
    const targetSign = ZODIAC_RULERS[zodiacNum] || ZODIAC_RULERS[1];

    // คำนวณดาวประจำวัน
    const dayPlanetMap = [1, 2, 3, 4, 5, 6, 7];
    const dayPlanet = dayPlanetMap[dayOfWeek] || 1;

    const startIdx = TAKSA_ORDER.indexOf(dayPlanet);
    const targetIdx = TAKSA_ORDER.indexOf(targetSign.ruler);

    let offset = targetIdx - startIdx;
    if (offset < 0) offset += 8;

    // คำนวณ Variation ประจำสัปดาห์
    const firstDayOfYear = new Date(dateObj.getFullYear(), 0, 1);
    const pastDaysOfYear = (dateObj - firstDayOfYear) / 86400000;
    const weekNumber = Math.ceil((pastDaysOfYear + firstDayOfYear.getDay() + 1) / 7);
    const variationIdx = weekNumber % 3;

    const taksaData = TAKSA_MEANINGS[offset];
    const currentVar = taksaData.variations[variationIdx];

    return {
        signName: targetSign.name,
        icon: targetSign.icon,
        taksaName: taksaData.name,
        description: currentVar.desc,
        good: currentVar.good,
        bad: currentVar.bad
    };
}

/**
 * 📝 จัดรูปแบบข้อความพยากรณ์มงคลประจำวัน
 */
function formatFortuneMessage(zodiacNum, fortune) {
    ...
    if (typeof fortune === 'object') {
        return `📿 [พยากรณ์ดวงชะตา ${fortune.icon} ${fortune.signName}]\n` +
               `📅 ประจำ${today}\n\n` +
               `🌟 ตกภูมิ: ${fortune.taksaName}\n` +
               `🔮 คำทำนาย: ${fortune.description}\n\n` +
               `✅ ข้อดีเด่น: ${fortune.good}\n` +
               `⚠️ ข้อควรระวัง: ${fortune.bad}\n\n` +
               `✨ สยามโหรามงคล สำนักพยากรณ์ศาสตร์ชั้นสูง`;
    }

    return `📿 [พยากรณ์ดวงชะตา]\n📅 ประจำ${today}\n\n${fortune}\n\n✨ สยามโหรามงคล`;
}
