...
const DASH_DAY_COLORS = {
    "อาทิตย์": { colors: ["แดง", "ชมพู"], bg: "#e74c3c" },
    "จันทร์": { colors: ["เหลือง", "เขียว"], bg: "#f1c40f" },
    "อังคาร": { colors: ["ชมพู", "ม่วง"], bg: "#e67e22" },
    "พุธ": { colors: ["เขียว", "ส้ม"], bg: "#2ecc71" },
    "พฤหัสบดี": { colors: ["ส้ม", "ฟ้า"], bg: "#e67e22" },
    "ศุกร์": { colors: ["ฟ้า", "ขาว"], bg: "#3498db" },
    "เสาร์": { colors: ["ม่วง", "ดำ"], bg: "#9b59b6" }
};
...
function renderWeeklyFortunePreview() {
    ...
    // คำนวณหา วันมงคลสูงสุด และ วันระวัง ด้วยกาลโยคจริงของสัปดาห์ / วันนั้น
    // calculateKalaYok จะให้ค่าดัชนีวัน (0=อาทิตย์, 1=จันทร์...) ของกาลโยค
    ...
    if (typeof calculateKalaYok === 'function') {
        ...
        if (bestSelect === 'auto') {
            bestDayText = `${dayNamesMap[kala.thongChai]} (ธงชัย) / ${dayNamesMap[kala.athibadi]} (อธิบดี)`;
        } else {
        ...
        if (badSelect === 'auto') {
            badDayText = `${dayNamesMap[kala.ubart]} (อุบาทว์) / ${dayNamesMap[kala.lokawinat]} (โลกาวินาศ)`;
        } else {
            const selectMap = { sun: "วันอาทิตย์", mon: "วันจันทร์", tue: "วันอังคาร", wed: "วันพุธ", thu: "วันพฤหัสบดี", fri: "วันศุกร์", sat: "วันเสาร์" };
    ...
    area.innerHTML = `
        <div id="wfCaptureArea" style="background: #fff; width: 1200px; height: 800px; position: relative; font-family: 'Sarabun', sans-serif; overflow: hidden;">
            ...
            <!-- Top Highlights -->
            <div style="display: flex; justify-content: center; gap: 40px; margin-top: 40px; padding: 0 50px;">
                <!-- วันมงคลสูงสุด -->
                <div style="width: 250px; height: 250px; border-radius: 50%; background: linear-gradient(135deg, #f1c40f, #e67e22); padding: 5px; box-shadow: 0 10px 20px rgba(0,0,0,0.1);">
                    <div style="background: #fff; width: 100%; height: 100%; border-radius: 50%; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; padding: 20px;">
                        ...
                        <div style="font-size: 18px; font-weight: bold; color: #333; margin-bottom: 5px;">วันมงคลสูงสุด</div>
                        <div style="font-size: 14px; color: #666; line-height: 1.4;">(ธงชัย/อธิบดี)</div>
                        <div style="font-size: 15px; color: #d4af37; font-weight: bold; margin-top: 10px; width: 100%; max-width: 180px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${bestDayText}">${bestDayText}</div>
                    </div>
                </div>
                ...
                <div style="width: 250px; height: 250px; border-radius: 50%; background: linear-gradient(135deg, #e74c3c, #c0392b); padding: 5px; box-shadow: 0 10px 20px rgba(0,0,0,0.1);">
                    <div style="background: #fff; width: 100%; height: 100%; border-radius: 50%; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; padding: 20px;">
                        ...
                        <div style="font-size: 18px; font-weight: bold; color: #333; margin-bottom: 5px;">วันที่ต้องระวัง</div>
                        <div style="font-size: 14px; color: #666; line-height: 1.4;">(อุบาทว์/โลกาวินาศ)</div>
                        <div style="font-size: 15px; color: #e74c3c; font-weight: bold; margin-top: 10px; width: 100%; max-width: 180px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${badDayText}">${badDayText}</div>
                    </div>
                </div>
            </div>
...
async function downloadWeeklyFortuneImage() {
    ...
    try {
        ...
        if (typeof calculateKalaYok === 'function') {
            ...
            if (bestSelect === 'auto') {
                bestDayText = `${dayNamesMap[kala.thongChai]} / ${dayNamesMap[kala.athibadi]}`;
            } else {
                const selectMap = { sun: "วันอาทิตย์", mon: "วันจันทร์", tue: "วันอังคาร", wed: "วันพุธ", thu: "วันพฤหัสบดี", fri: "วันศุกร์", sat: "วันเสาร์" };
                bestDayText = selectMap[bestSelect] || "วันอาทิตย์";
            }
            ...
            if (badSelect === 'auto') {
                badDayText = `${dayNamesMap[kala.ubart]} / ${dayNamesMap[kala.lokawinat]}`;
            } else {
                const selectMap = { sun: "วันอาทิตย์", mon: "วันจันทร์", tue: "วันอังคาร", wed: "วันพุธ", thu: "วันพฤหัสบดี", fri: "วันศุกร์", sat: "วันเสาร์" };
                badDayText = selectMap[badSelect] || "วันเสาร์";
            }
        ...
        ctx.fillText(bestDayText, 205, highlightY + 175);
        ...
        ctx.fillStyle = '#666';
        ctx.font = '16px "Sarabun", sans-serif';
        ctx.fillText("(อุบาทว์/โลกาวินาศ)", 1200 - 205, highlightY + 145);
        ctx.fillStyle = '#e74c3c';
        ctx.font = 'bold 15px "Sarabun", sans-serif';
        ctx.fillText(badDayText, 1200 - 205, highlightY + 175);
