...
function openVIPReportModal() {
    const existing = document.getElementById('vipReportModal');
    if (existing) existing.remove();

    let allHistory = JSON.parse(localStorage.getItem("horo_history")) || [];
    let memberOptions = '<option value="">-- เลือกจากฐานข้อมูลลูกค้า --</option>';
    // กรองเอาเฉพาะข้อมูลที่มีชื่อ
    allHistory.filter(m => m.name).forEach((m, idx) => {
        // หา idx จริงๆ ในอาร์เรย์เดิม
        let realIdx = allHistory.indexOf(m);
        memberOptions += `<option value="${realIdx}">${m.name} (${m.birthdate || 'ไม่ระบุวันเกิด'})</option>`;
    });
    ...
    let provinceOptions = '<option value="">-- ไม่ระบุ --</option>';
    THAI_PROVINCES.forEach(p => {
        provinceOptions += `<option value="${p}">${p}</option>`;
    });

    const html = `
    <div id="vipReportModal" class="admin-modal" style="display: flex; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); z-index: 10000; align-items: center; justify-content: center;">
        <div class="admin-modal-content" style="background: #1a1a1a; border: 1px solid #d4af37; border-radius: 10px; width: 100%; max-width: 600px; max-height: 90vh; overflow-y: auto; color: black; padding: 20px;">
            <div class="d-flex justify-content-between align-items-center border-bottom border-gold pb-2 mb-3">
                <h3 class="text-gold m-0">📄 สร้างรายงานดวงชะตารายเดือน (VIP)</h3>
                <span onclick="document.getElementById('vipReportModal').remove()" style="font-size: 1.5rem; cursor: pointer; color: #888;">&times;</span>
            </div>
            
            <div class="form-group mt-3" style="background: rgba(212, 175, 55, 0.1); padding: 15px; border-radius: 8px;">
                <label style="color: #D4AF37;">🔍 ดึงข้อมูลจากระบบสมาชิก:</label>
                <select id="vipMemberSelect" class="form-control bg-dark text-white border-gold" onchange="window.autoFillVipMember(this.value)">
                    ${memberOptions}
                </select>
                <small class="text-muted mt-1 d-block">* หากเลือกจากระบบ ข้อมูลด้านล่างจะถูกกรอกอัตโนมัติ</small>
            </div>
            
            <div style="display: none;">
                <input type="text" id="vipName">
                <input type="text" id="vipIdCard">
                <input type="date" id="vipDobDate">
                <input type="time" id="vipDobTime">
                <select id="vipLocation">
                    ${provinceOptions}
                </select>
            </div>
            ...
            <div class="form-group mt-3">
                <label>🗓️ เลือกเดือน-ปี ที่ต้องการสร้างคำทำนาย:</label>
                <input type="month" id="vipTargetMonth" class="form-control bg-dark text-white border-gold">
            </div>

            <button class="btn btn-primary btn-block mt-4" id="btnGenerateVIP" onclick="generateVIPReport()" style="font-size: 1.1rem; padding: 12px; background: #d4af37; color: black; border: none; font-weight: bold;">
                <i class="fas fa-file-pdf"></i> เริ่มประมวลผลและดาวน์โหลด PDF
            </button>
            
            <div id="vipPdfStatus" class="mt-3 text-center text-info" style="display:none; font-size: 18px;">
                <div><i class="fas fa-spinner fa-spin"></i> ระบบกำลังประมวลผลและสร้างไฟล์ PDF (34 หน้า)</div>
                <div id="vipPdfSubText" style="font-size: 14px; margin-top: 5px; color: #aaa;">ใช้เวลาประมาณ <span id="vipPdfTimer">15</span> วินาที...</div>
            </div>
        </div>
    </div>
    `;
    ...
    document.getElementById('vipTargetMonth').value = mStr;
}

window.autoFillVipMember = function (indexStr) {
    if (!indexStr) {
        // Clear if unselected
        document.getElementById('vipName').value = '';
        document.getElementById('vipIdCard').value = '';
        document.getElementById('vipDobDate').value = '';
        document.getElementById('vipDobTime').value = '';
        document.getElementById('vipLocation').value = '';
        return;
    }
    let allHistory = JSON.parse(localStorage.getItem("horo_history")) || [];
    let m = allHistory[parseInt(indexStr)];
    if (m) {
        let fullName = m.name || '';
        if (m.lastName) fullName += ' ' + m.lastName;
        document.getElementById('vipName').value = fullName.trim();
        document.getElementById('vipIdCard').value = m.ThaiId || m.idCard || '';
        ...
        if (m.birthdate) {
            try {
                ...
                if (parts.length === 3) {
                    ...
                    document.getElementById('vipDobDate').value = `${year}-${month}-${day}`;
                }
        ...
        document.getElementById('vipLocation').value = m.birthLocation || m.location || '';
    }
}

// Real Astrological Daily Fortune Generator
    ...
    const navHTML = `
        <div id="vipPdfPreviewNav" style="position: sticky; top: 0; background: #F8F9FA; padding: 15px; border-bottom: 2px solid #D4AF37; display: flex; justify-content: space-between; align-items: center; z-index: 100001; box-shadow: 0 4px 6px rgba(0,0,0,0.3);">
            <div style="color: #D4AF37; font-size: 20px; font-weight: bold;">
                📄 พรีวิวรายงานดวงชะตา (VIP)
            </div>
            <div>
                <button class="btn btn-warning mr-2" onclick="window.print()" style="font-size: 16px; font-weight: bold;">
