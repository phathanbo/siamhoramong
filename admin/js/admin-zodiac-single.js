...
async function postToFacebook() {
    if (typeof generateDailyZodiacFortunes !== 'function') return;
    const zodiacIndex = parseInt(document.getElementById('zdZodiacSelect').value, 10);
    const dateStr = document.getElementById('zdDate').value;
    if (!dateStr || isNaN(zodiacIndex)) return;
    ...
    Swal.fire({
        ...
        didOpen: async () => {
            ...
            try {
                ...
                const fbPreview = document.getElementById('fbTextPreview');
                const msg = (fbPreview && fbPreview.value) ? fbPreview.value : `ดวงรายวันราศี${p.name}`;
            ...
            `,
                ...
                console.error(err);
                Swal.fire('ข้อผิดพลาด', 'ไม่สามารถโพสต์ได้: ' + err.message, 'error');
            }
        }
    });
}
