...
function displayFengShuiCalendar() {
    ...
    if (resultEl) {
        // ใช้ birthDay เพื่อคำนวณดาวประจำตัว (personalized)
        const personifiedPlanetNum = (birthDay % 9) || 9;
        const personifiedPlanet = PLANET_DIRECTION[personifiedPlanetNum];

        // ใช้ธาตุประจำปีของบุคคล (ไม่ใช่เดือน)
        ...
        const deepLevelInfo = `
            ...
            <div class="alert alert-info small mb-3">
                <strong>📊 พื้นฐาน:</strong> ดาวเกิด (${personifiedPlanetNum}) + ธาตุประจำปี (${element.name}) + ทิศหลังบ้าน (${sittingDir})
            </div>
