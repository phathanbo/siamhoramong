...
function getDithee(date) {
  if (typeof getThaiLunar === 'function') {
    const lunar = getThaiLunar(date);
    if (lunar) {
      const phase = lunar.phase === "ข้างขึ้น" ? "ขึ้น" : "แรม";
      const phaseThai = lunar.phase === "ข้างขึ้น" ? "ข้างขึ้น (ศุกลปักษ์)" : "ข้างแรม (กฤษณปักษ์)";
      const day = lunar.amount;
      const ditheeNum = lunar.phase === "ข้างขึ้น" ? lunar.amount : lunar.amount + 15;
      
      const lunarAge = (ditheeNum - 1) * (29.53 / 30);
      const moonPct = Math.round((lunarAge / 29.53) * 100);
  ...
  // Fallback กรณีไม่มี getThaiLunar
