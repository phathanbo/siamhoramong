    ...
    if (selectedDay !== "") {
        ...
        if (currentMemberIdForAnalysis) {
            ...
            if (profileData && profileData.birthdate) {
                const parseFn = typeof parseThaiDate === 'function' ? parseThaiDate : (typeof window !== 'undefined' && typeof window.parseThaiDate === 'function' ? window.parseThaiDate : (typeof parseBirthdate === 'function' ? parseBirthdate : (d) => new Date(d)));
                const birthDate = parseFn(profileData.birthdate);
                if (birthDate) {
